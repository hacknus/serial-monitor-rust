                            ...
                            for graph_idx in 0..self.serial_devices.number_of_plots[self.device_idx] {
                                ...
                                let plot_inner = signal_plot.show(ui, |signal_plot_ui| {
                                    for (i, graph) in graphs.iter().enumerate() {
                                        ...
                                        if i < self.serial_devices.plot_labels[self.device_idx].len() {
                                            signal_plot_ui.line(
                                                Line::new(PlotPoints::Owned(graph.to_vec())).name(
                                                    &self.serial_devices.plot_labels[self.device_idx][i],
                                                ),
                                            );
                                        }
                                    }
    ...
    fn draw_plot_settings(&mut self, ctx: &egui::Context, ui:&mut Ui) {
        ...
        ui.horizontal( |ui| {
            ...
            if ui.button("Reset Labels").clicked() {
                self.serial_devices.plot_labels[self.device_idx] = self.data.names.clone();
            }
        });
        ...
        for i in 0..self.data.names.len().min(10) {
            ...
            if self.serial_devices.plot_labels[self.device_idx].len() <= i {
                break;
            }

            if ui.add(
                egui::TextEdit::singleline(&mut self.serial_devices.plot_labels[self.device_idx][i])
                        ...
                        match self.show_warning_window {
                            ...
                            WindowFeedback::Clear => {
                                ...
                                if !device_is_already_saved {
                                    ...
                                    self.serial_devices.plot_labels.push(vec!["Column 0".to_string()]);
